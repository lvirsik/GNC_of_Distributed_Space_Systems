classdef dynamics
    methods (Static)
        ...
        function statedot = two_body_dynamics_control(t, state, simulation_settings, control_acc)
            % Take in state vector and return statedot vector (accelerations)
            r = state(1:3);
            v = state(4:6);
            ...
            r_dot = v;
            v_dot = a + control_acc;
            % if norm(control_acc) > 0.1
            %     fprintf('Applying control at t = %.2f s, accel = [%.3e %.3e %.3e] m/s²\n', t, control_acc);
            % end
